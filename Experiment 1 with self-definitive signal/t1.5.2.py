...
if __name__ == '__main__':
    ...
    for i in range(len(thsmatrix)):
        for j in range(len(ods)):
            ...
            ths = thsmatrix[i]
            if i == 3:
                ncoeffs = dwd.thprocess2(coeffs, ths=thsmatrix[i][j], thf='thf3', thps=[2 + 2 * k for k in range(dl)])
            else:
                ncoeffs = dwd.thprocess(coeffs, ths=thsmatrix[i][j], thf=thfname[i], thp=4)
